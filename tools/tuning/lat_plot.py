...
def old_feedforward(speed, angle):
  ...
  if IS_ANGLE_PLOT:
    ...
    return np.vectorize(get_steer_feedforward_volt_old)(angle, speed)
    # return 0.000195 * (speed ** 2) * angle # old suburban
    # return 0.0002 * (speed ** 2) * angle # old bolt and bolteuv
    # return 0.00004 * (speed ** 2) * angle # old silverado/sierra
  else:
...
def feedforward(speed, angle, ANGLE_COEF, ANGLE_COEF2, ANGLE_OFFSET, SPEED_OFFSET, SIGMOID_COEF_RIGHT, SIGMOID_COEF_LEFT, SPEED_COEF):
  if IS_ANGLE_PLOT:
    ...
    return np.vectorize(get_steer_feedforward_bolt)(angle, speed, ANGLE_COEF, ANGLE_COEF2, ANGLE_OFFSET, SPEED_OFFSET, SIGMOID_COEF_RIGHT, SIGMOID_COEF_LEFT, SPEED_COEF)
  else:
    ...
    # suburban
    return np.vectorize(get_steer_feedforward_bolt_torque)(angle, speed, ANGLE_COEF, ANGLE_COEF2, ANGLE_OFFSET, SPEED_OFFSET, SIGMOID_COEF_RIGHT, SIGMOID_COEF_LEFT, SPEED_COEF)
...
def fit(speed, angle, steer, angle_plot=True):
  ...
  params, _ = curve_fit(  # lgtm[py/mismatched-multiple-assignment] pylint: disable=unbalanced-tuple-unpacking
    ...
    bounds=([0.001, 0.01, -90., 0., 0.3, 0.3, 0.001],
        [5.0, 2.0, 90., 20., 5., 5.0, 10.]) if IS_ANGLE_PLOT else \
        ([0.1, 0.1, -3., -20., 0.2, 0.2, 0.01],
        [5.0, 2.0, 3., 20., 2.0, 2.0, 2.0])
